...
def rgb_ortho():
    for item in dirs:
        ...
        if os.path.isfile(path+item):
            ...
            red_areas = (red > blue) & (red > green) & np.logical_not(black_areas)
            green_areas = (green > blue) & (green > red) & np.logical_not(black_areas)
            yellow_areas = (red > 70) & (green > 70) & (blue < 50)
            blue_areas = (blue > 70)
            ...
            data[..., :-1][green_areas.T] = (255, 0, 1) # Blue channel as class index
            data[..., :-1][blue_areas.T] = (0, 255, 2) # Blue channel as class index
            data[..., :-1][yellow_areas.T] = (255, 255, 3) # Blue channel as class index
    ...
    pixels = np.sum(dist)
    #distribution = (dist[0]/pixels, dist[1]/pixels, dist[2]/pixels, dist[3]/pixels, dist[4]/pixels, pixels)
    #print(distribution)
    #with open(path + "distribution.txt", "w") as text_file:
        #text_file.write("black: %f red: %f green: %f yellow: %f blue: %f pixels: %d" % distribution)
